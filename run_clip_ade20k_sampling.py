"""
    Running on ade20k, over-/under-sampling configuration
"""
...
if __name__ == '__main__':
    ...
    # oversampling 
    if target_elems > label_counts[0]:
        df_sampled = df_train.groupby('tags', group_keys=False).apply(lambda x: x.sample(replace=True, n=target_elems-len(x)))
        df_sampled = pd.concat([df_train, df_sampled], ignore_index=True)
    # undersampling
    else:
        df_sampled = df_train.groupby('tags', group_keys=False).apply(lambda x: x.sample(replace=True, n=target_elems))
    ...
    filename = f'ade20k_clip_sampling_{target_elems}'
